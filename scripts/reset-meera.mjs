// scripts/reset-meera.mjs
//
// ONE-COMMAND DEMO RESET. Returns the "Meera Nair" demo patient to a clean
// pre-demo baseline so every run of the demo looks identical:
//   • profile: Meera Nair, DOB 1989-03-14, B+
//   • exactly 3 health facts (1 condition, 1 medication, 1 allergy)
//   • exactly 3 reports, all ai_status='done' (re-uploaded + re-inserted)
//   • shares = 0  (deleting shares CASCADE-clears access_log → views = 0)
//   • profiles.summary = a freshly generated at-a-glance handoff
//
// Run with:  node scripts/reset-meera.mjs
//
// Uses the PUBLIC anon key + Meera's own login — everything is owner-scoped, RLS
// enforced, exactly like the real app. No service-role key needed. Idempotent.
// If the account still needs email confirmation it prints NEEDS_CONFIRM and exits 2.

import { readFileSync } from 'node:fs';
import { createClient } from '@supabase/supabase-js';

// ── Demo credentials (throwaway demo account; safe to commit) ─────────────────
...
// ── Canonical demo content (the baseline every reset restores) ────────────────
const FACTS = [
...
async function main() {
  ...
  // 2. Shares → delete first. The FK access_log.share_id ON DELETE CASCADE means
  //    clearing shares also clears every access_log row (views → 0).
  const { error: sErr } = await supabase.from('shares').delete().eq('user_id', userId);
  if (sErr) throw sErr;
  console.log('Shares cleared (access_log cascades to 0).');

  // 3. Facts → reset to the canonical 3.
  await supabase.from('health_facts').delete().eq('user_id', userId);
  const { error: fErr } = await supabase
    .from('health_facts')
    .insert(FACTS.map((f) => ({ user_id: userId, ...f })));
  if (fErr) throw fErr;
  console.log('Facts reset:', FACTS.length);

  // 4. Records → wipe any existing (incl. files uploaded during rehearsal), then
  //    re-seed the canonical 3 with their persisted AI explanations.
  const { data: existing } = await supabase
    .from('health_records')
    .select('id, file_path')
    .eq('user_id', userId);
  if (existing && existing.length > 0) {
    const paths = existing.map((r) => r.file_path).filter(Boolean);
    if (paths.length) await supabase.storage.from('health-docs').remove(paths);
    await supabase.from('health_records').delete().eq('user_id', userId);
    console.log('Old records + files removed:', existing.length);
  }
  ...
  // 5. Regenerate the at-a-glance handoff (one Claude call) and persist it onto
  //    profiles.summary. The clinic view reads this as "atAGlance".
  const { data: sum, error: sumErr } = await supabase.functions.invoke('patient-summary', {
    body: {},
  });
  if (sumErr) {
    console.warn('Summary regen failed (non-fatal):', sumErr.message ?? sumErr);
  } else if (sum?.summary) {
    console.log('At-a-glance summary regenerated:\n  ', sum.summary.slice(0, 120) + '…');
  } else {
    console.warn('Summary regen returned empty — check ANTHROPIC_API_KEY function secret.');
  }

  console.log('\n✅ Reset complete. Demo login:');
  console.log('   email:   ', DEMO_EMAIL);
  console.log('   password:', DEMO_PASSWORD);
  console.log('   state:    3 facts · 3 reports (done) · shares=0 · views=0 · summary set');
}
